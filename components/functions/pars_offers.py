...
def pars_file(jsons_class: JsonList) -> None:
    ...
    for event, elem in et.iterparse(path, events=('end',), tag=['offer'], recover=True):
        if event == 'end' and elem.tag == 'offer':
            idd: str = elem.find("group_id").text.strip()
            url: str = (f'http://market.apisystem.name/models/{idd}/offers?&format=json&api_key'
                        f'=0c8bd89c2e517164204ceb1dfee65289a659b4bcffee#')
            teeeemp = get_json_from_url(url)
            if 'filters' in teeeemp:
                print(idd)
                break
        # if event == 'end' and elem.tag == 'offer':
        #     temp = {}
